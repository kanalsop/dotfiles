...
未作成のプロファイルは、拡張機能のインストール前に `code --new-window --profile` で作成します。その際は VS Code のウィンドウが開きます。保存を最大30秒待ち、タイムアウトした場合は VS Code の起動を完了させてから再実行してください。既存のプロファイルは再作成しません。
